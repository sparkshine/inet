...
void SCTPAssociation::printAssocBrief()
{
    ...
    EV_DETAIL << localAddr << ":" << localPort << " to " << remoteAddr << ":" << remotePort;
    EV_DETAIL << "  on app[" << appGateIndex << "],assocId=" << assocId;
    EV_DETAIL << "  in " << stateName(fsm->getState()) << "\n";
}

void SCTPAssociation::printSegmentBrief(SCTPMessage *sctpmsg)
{
    //EV_DETAIL << "." << sctpmsg->getSrcPort() << " > ";
    //EV_DETAIL << "." << sctpmsg->getDestPort() << ": ";
    //EV_DETAIL << "initTag "<< sctpmsg->getTag() << "\n";
}
...
void SCTPAssociation::sendInit()
{
    ...
    if (remoteAddressList.size()>0)
    {
        ...
        {
            EV_DEBUG<<__LINE__<<" get new path for "<<(*it)<<"\n";
    ...
    {
        EV_DEBUG<<__LINE__<<" get new path for "<<remoteAddr<<"\n";
...
void SCTPAssociation::addPath(const Address& addr)
{
    ...
    {
        EV_DEBUG<<__LINE__<<" get new path for "<<addr<<"\n";
...
SCTPPathVariables* SCTPAssociation::getNextDestination(SCTPDataVariables* chunk) const
{
    ...
    EV_INFO << "getNextDestination(): chunk was last sent to " << last->remoteAddress

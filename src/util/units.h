...
// W = pow(10, dBm / 10) / 1000
// dBm = 10 * log(10, W * 1000)
...
namespace units
{
    ...
    // Constructs a unit equivalent to log(Unit, Num/Den)
    template<typename Unit, int Num, int Den=1>
    struct log;
...
namespace units
{
	...
	namespace units
	{
		...
		// TODO: typedef scale<log<unit, 10>, 10> dB;
		typedef scale<pow<unit, 1>, 10> dB;
        // TODO: typedef scale<log<scale<unit, 1000>, 10>, 10> dBm;
        typedef scale<pow<scale<unit, 1000>, 1>, 10> dBm;

		// non-SI power
		typedef scale<W, 1000> mW;
	}
    ...
    UNIT_DISPLAY_NAME( units::dB,       "dB" );
    UNIT_DISPLAY_NAME( units::dBm,      "dBm" );
	...
	namespace values
	{
        ...
        typedef value<double, units::dB> dB;
        typedef value<double, units::dBm> dBm;
	}
